  ...
  return (
    <InView onChange={inView => lastProductElement(inView)}>
      <Card style={index % 2 === 0 ? styles.shoppingCardOdd : styles.shoppingCardEven}
        header={() => CardHeader({ picture: `https://www.trike.com.ph${product.thumbnail}` })}
      ...
      >
        {product.cheapest_variant.sale_price_active ? (
          ...
          <Text style={{ color: '#FAA634', fontWeight: '700', fontSize: 17}}>{product.cheapest_variant.price ? `₱ ${product.cheapest_variant.price.toFixed(2)}` : ''}</Text>
        )}
